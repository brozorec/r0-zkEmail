...
impl EmailVerifier {
    ...
    pub async fn verify_email(
        ...
        for header in dkim_headers.iter() {
            ...
            match retrieve_public_key(&logger, self.resolver.clone(), from_domain.to_string(), selector)
                .await
            {
                Ok(pk) => {
        ...
        let result = verify_email_with_resolver(&logger, from_domain, &email, self.resolver.clone())
            .await
    ...
    fn generate_and_verify_proof(&self, email: &Email) -> Result<DKIMOutput> {
        ...
        let _groth16_receipt = self.convert_to_groth16(&succinct_receipt)?;

        Ok(output)
    }

    fn convert_to_groth16(&self, receipt: &risc0_zkvm::Receipt) -> Result<risc0_zkvm::Receipt> {
